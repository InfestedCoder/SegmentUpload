...
async function realizeSegment(priorDrawingId, priorPlacement) {
...
async function realizeDrawing(priorDrawingId, priorPlacement) {
...
const reset = (priorDrawingId, priorPlacement) => {
    ...
    segmentPromise.then((segment) => {
        document.getElementById('currentSegment').innerHTML = 'You are creating the ' + segment.placement + ' of drawing id ' + segment.drawing.id;
    });
